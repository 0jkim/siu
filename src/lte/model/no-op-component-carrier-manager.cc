...
// Configured Grant
void
NoOpComponentCarrierManager::DoUlReceiveCgr (uint16_t rnti, uint8_t componentCarrierId, uint32_t bufSize, uint8_t lcid)
{
  NS_LOG_FUNCTION (this);
  ...
  sapIt->second->ReportCgrToScheduler (rnti, bufSize, lcid);
}
...
// Configured Grant
void
RrComponentCarrierManager::DoUlReceiveCgr(uint16_t rnti, [[maybe_unused]] uint8_t componentCarrierId, uint32_t bufSize, uint8_t lcid)
{
  NS_LOG_FUNCTION (this);
  // split traffic in uplink equally among carriers
  uint32_t numberOfCarriersForUe = m_ueInfo.at (rnti).m_enabledComponentCarrier;

  m_ccmMacSapProviderMap.find (m_lastCcIdForSr)->second->ReportCgrToScheduler (rnti, bufSize, lcid);
