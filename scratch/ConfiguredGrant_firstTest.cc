...
public:
  ...
  void Setup (Ptr<NetDevice> device, Address address, uint32_t packetSize, uint32_t nPackets,
              DataRate dataRate, uint8_t period, uint32_t deadline, bool period_on, uint32_t seed);
...
MyModel::SendPacketUl ()
{
  ...
  m_device->Send (pkt, m_addr,
                  Ipv4L3Protocol::PROT_NUMBER); // Send로 Mac 계층에 pkt(생성한 패킷)을 보냄
  ...
  for (uint32_t i = 0; i < ueNumPergNb; i++) // setup 메서드로 UE의 패킷 시나리오 파라미터 구축
    {
      ...
      std::uniform_int_distribution<> distr_packet_size (10,
                                                         500); // 패킷 사이즈 10~500 bytes로 할당
      std::uniform_int_distribution<> distr_init (50000,
                                                  200000); // 초기 패킷 생성 시점 랜덤 값 부여
      std::uniform_int_distribution<> distr_period (
          10, 5000); // 패킷 생성 주기 랜덤 값 부여 (계속 이 주기로 패킷 생성 예약됨)
  ...
  for (uint8_t ii = 0; ii < ueNumPergNb; ++ii)
    {
      ...
      modelUl->Setup (ueNetDev.Get (ii), enbNetDev.Get (0)->GetAddress (), v_packet[ii], nPackets,
                      DataRate ("1Mbps"), v_period[ii], v_deadline[ii], period_on, seed);
